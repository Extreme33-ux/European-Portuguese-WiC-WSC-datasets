...
# Links for the MultiRC task
train_file = "https://huggingface.co/datasets/PORTULAN/extraglue/resolve/main/data/multirc_pt-PT/train.jsonl"
val_file = "https://huggingface.co/datasets/PORTULAN/extraglue/resolve/main/data/multirc_pt-PT/validation.jsonl"
os.environ["CUDA_VISIBLE_DEVICES"] = "5"  # Change this depending on the GPU to use
...
def tokenize_function(example):
    ...
    for paragraph, question,answer in zip(example['paragraph'], example['question'], example['answer']):
        concatenated_examples['conc'].append(paragraph)
        concatenated_examples['question_answer'].append(f"{question} [SEP] {answer}")

    tokenized_input = tokenizer(
        text = concatenated_examples['conc'],
        text_pair = concatenated_examples['question_answer'],
        padding = True,
        truncation = True,
    )
