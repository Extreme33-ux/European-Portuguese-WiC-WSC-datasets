...
train_file = "https://huggingface.co/datasets/PORTULAN/extraglue/resolve/main/data/copa_pt-PT/train.jsonl"
val_file = "https://huggingface.co/datasets/PORTULAN/extraglue/resolve/main/data/copa_pt-PT/validation.jsonl"
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # Change this depending on the GPU to use

# Load the dataset from the JSONL files
dataset = load_dataset('json', data_files={'train': train_file, 'validation':val_file})
checkpoint = "PORTULAN/albertina-ptpt"
...
def tokenize_function(example):
    ...
    tokenized_choice1 = tokenizer(example['premise'], example['choice1'], padding = True, truncation = True)
    tokenized_choice2 = tokenizer(example['premise'], example['choice2'], padding = True, truncation = True)
    
    concatenated_examples = {
    'conc': [],
    'question': []
    }
    for premise, choice1, choice2, question in zip(example['premise'], example['choice1'], example['choice2'], example['question']):
        concatenated_examples['conc'].append(f"{premise} [SEP] {choice1} [SEP] {choice2}")
        concatenated_examples['question'].append(question)

    tokenized_input = tokenizer(
        text = concatenated_examples['conc'],
        text_pair = concatenated_examples['question'],
        padding = True,
        truncation = True,
    )
